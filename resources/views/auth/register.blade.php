...
<div class="container-fluid">
    <div class="row">
        <div class="col-sm-6">
            <form method="POST" action="{{ route('post-register') }}" id="form_register">
            @csrf
                ...
                <div class="form">
                    <h3>Create account at Nodigy</h3>
                    <p class="subtext">You will need to confirm your email address</p>
                    ...
                    <div class="form-group is_agree">
                        <div class="cus_checkbox bycreatingtext">
                            <input class="form-check-input" type="checkbox" name="is_agree" id="is_agree" {{ old('is_agree') ? 'checked' : '' }}><span>By creating an account, you agreeing to our Privacy Policy</span>
                        </div>
                        @error('is_agree')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                    ...
                    <div class="donthaveacount">Already have an account? <a href="{{route('login')}}">Sign In</a></div>
                </div>
...
$(document).ready(function() {
    isAgree();
    $('body').on("change", "#is_agree", function(event){
        isAgree();
    });
    function isAgree(){
    ...
    $('#form_register').on('keyup change','input, textarea, select',function(event) {
    ...
    $("input[name='password_confirmation']").keypress(function(e) {
        if(e.which === 32){
            return false;
        }
    });
